...
def remove_item(user, product, number_unit):
    ...
    if users_cart[user][product][1] < 1:
...
stop_all = False
while not stop_all:
    user_name = input("Please enter The Shopper's name: ").title()
    ...
    while not stop:
        ...
        if user_decision == "5":
            ...
            while True:
                show_item(user_name)
                show_stop = input("Please enter s to return to menu: ")
                if show_stop == "s":
                    break
            ...
            while True:
                show_item(user_name)
                print(f"your cart cost is: {cart_price(user_name)}")
                price_stop = input("Please enter s to return to menu: ")
                if price_stop == "s":
                    break
        else:
            os.system("clear")
        ...
        exit_program = input("Press any key to add another user and exit to exit program: ")
